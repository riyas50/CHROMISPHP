...
function filterRecords($CUSTNAME,$PRODUCT) 
    {
        ...
        //========================================================================================================
        //below code generated with workbench plugin for php under tools > utilities
        $query = "(select *,'MAX-BILLED' `STATUS` ";
        $query = $query . " from view_customer_item_search ";
        $query = $query . " WHERE NAME LIKE \"%$CUSTNAME%\" AND PRODUCT LIKE \"%$PRODUCT%\" AND UNITPRICE = ";
        $query = $query . " (select MAX(UNITPRICE) ";
        $query = $query . "  from view_customer_item_search ";
        $query = $query . "  WHERE NAME LIKE \"%$CUSTNAME%\" AND PRODUCT LIKE \"%$PRODUCT%\")";
        $query = $query . " LIMIT 1)";
        $query = $query . " UNION ";
        $query = $query . " (select *,'MIN-BILLED' `STATUS` ";
        $query = $query . " from view_customer_item_search ";
        $query = $query . " WHERE NAME LIKE \"%$CUSTNAME%\" AND PRODUCT LIKE \"%$PRODUCT%\" AND UNITPRICE = ";
        $query = $query . " (select MIN(UNITPRICE) ";
        $query = $query . "  from view_customer_item_search ";
        $query = $query . "  WHERE NAME LIKE \"%$CUSTNAME%\" AND PRODUCT LIKE \"%$PRODUCT%\")";
        $query = $query . " LIMIT 1)";
        $query = $query . " UNION ";
        $query = $query . " (select *,'LAST-BILLED' `STATUS` ";
        $query = $query . "  from view_customer_item_search ";
        $query = $query . "  WHERE NAME LIKE \"%$CUSTNAME%\" AND PRODUCT LIKE \"%$PRODUCT%\" AND INVDATE = ";
        $query = $query . "    (select MAX(INVDATE) ";
        $query = $query . "  from view_customer_item_search ";
        $query = $query . "  WHERE NAME LIKE \"%$CUSTNAME%\" AND PRODUCT LIKE \"%$PRODUCT%\") LIMIT 1)";
        //$query = $query . " LIMIT 1 ";
        //$query = $query . " group by TICKETID,NAME,PRODUCT,QTY,TOTAL,INVDATE,INVAMOUNT";

                if ($stmt = $conn->prepare($query)) {
                $stmt->execute();
                $stmt->bind_result($TICKETID,$CUSTOMER,$PRODUCT,$QTY,$UNITPRICE,$TOTAL,$INVDATE, $INVAMOUNT,$STATUS);
